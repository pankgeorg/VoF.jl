...
| — | **Interface-compression flux** (interFoam `cAlpha·|u_r|·α(1-α)`) | MULES keeps a sharp interface over 5 s sloshing | ◐ implemented + swept 2026-06-11 (`c_α` kwarg, default 1; unit-tested). Sharpness ✅, mass ✅ — but at ρ=1000 wall-slam ANY sharp interface destabilizes the u-advecting coupling (survival 0.53→2.5 s as c_α 1→0.25; only fully-diffused c_α=0 survives 5 s). Violent flows: use `step_vof!`+`mass_repair` (sharp via clamp, RMS 12.7%). Remaining: gentle-case validation (sloshing/Kelvin). See `ShipFlow.jl/RESULTS-damBreak.md` |
