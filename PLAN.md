...
| 3 | Surface tension on rising bubble (Hysing)             | Layer 1 bubble test in published envelope  | ◐ CSF + height-function κ implemented + unit-tested 2026-06-11 (`curvature!`/`csf_force!`/`surface_tension`, force = σκ∇α/ρ_face via udf). Hysing case 1: **rise dynamics in envelope** (v_max −0.6 %, y_c +1.3 % @ N=128), **shape not** (c_min 0.60 vs 0.901, grid-converged; height-function κ leaves it unchanged → κ ruled out, cause isolated to the unmodeled tangential μ-jump stress of the kinematic uniform-ν formulation — a deliberate scope limit, see Non-goals). See `ShipFlow.jl/RESULTS-hysing.md`. Not ship-critical (σ matters for cavitation, not resistance) |
...
| — | **Interface-compression flux** (interFoam `cAlpha·|u_r|·α(1-α)`) | MULES keeps a sharp interface over 5 s sloshing | ✅ **done 2026-06-11** (`c_α` kwarg, default 1; unit-tested). Gentle sloshing @ ρ=1000: stable 5.5 periods, period +0.5% vs linear theory, mass 1.3e-3 %, interface 2.4× sharper than c_α=0 or clamp+repair (`ShipFlow.jl/RESULTS-sloshing.md`). Caveat: in *violent* wall-slam flows a sharp algebraic interface destabilizes the u-advecting coupling — use `step_vof!`+`mass_repair` there (`RESULTS-damBreak.md`) |
