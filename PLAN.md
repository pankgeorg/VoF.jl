...
| 3 | Surface tension on rising bubble (Hysing)             | Layer 1 bubble test in published envelope  | ◐ CSF implemented + unit-tested 2026-06-11 (`curvature!`/`csf_force!`/`surface_tension`, force = σκ∇α/ρ_face via udf). Hysing case 1: **rise dynamics in envelope** (v_max −0.6 %, y_c +1.3 % @ N=128), **shape not** (c_min 0.60 vs 0.901, grid-converged) — smoothed-CSF κ under-restores the rim + unmodeled tangential μ-jump stress. Fix path: height-function κ (Popinet 2009). See `ShipFlow.jl/RESULTS-hysing.md`. Not ship-critical (σ matters for cavitation, not resistance) |
